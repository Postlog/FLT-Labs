...
from models import DFA, RegexParser
...
class NFA(Type):
    ...
    def determinize(self):
        ...
        dfa = DFA.from_nfa(self)
        # return NFA.normalize_answer(dfa)
        return dfa
    ...
    def minimize(self):
        dfa = NFA.determinize(self)
        return DFA.minify(dfa)
...
if __name__ == "__main__":
    ...
    #Я в своих методах не указывал эпсилон правило через символ ε, поэтому заменяю их на ''
    copy_transitions = dict()

    for symbol in fa.input_symbols:
        if symbol == EPSILON:
            fa.input_symbols.remove(EPSILON)
            break

    for transitions in fa.transitions.keys():
        dop_dict = dict()
        for key in fa.transitions[transitions].keys():
            if key == EPSILON:
                values = fa.transitions[transitions].get(key)
                dop_dict[''] = values
            else:
                values = fa.transitions[transitions].get(key)
                dop_dict[key] = values
        copy_transitions[transitions] = dop_dict
